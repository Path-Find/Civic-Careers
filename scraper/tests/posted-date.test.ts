...
import { extractClosingDate, extractClosingDateStatus, normalizeActiveClosingDateStatus } from '../closing-date';
...
test('extracts source closing dates without treating Job End Date as a deadline', () => {
  ...
  assert.equal(extractClosingDate('Deadline to Apply: Aug. 23, 2026'), '2026-08-23');
  assert.equal(extractClosingDate('Please submit their application by August 28, 2026'), '2026-08-28');
  ...
  assert.equal(extractClosingDate('Posting Period: 17-AUG-2026 to 18-SEP-2026'), '2026-09-18');
...
test('normalizes an active posting without a usable date to the public fallback', () => {
  assert.deepEqual(normalizeActiveClosingDateStatus('A rendered posting body with no application deadline.'), {
    date: null,
    status: 'open_until_filled',
  });
});
