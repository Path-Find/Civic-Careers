...
**Engine built 2026-07-12** (`scraper/engines/peoplesoft.ts`) — click-and-walk pattern (click first result, use the page's own "Next Job" control to walk the rest, since there are no real per-job URLs on this platform — every link is a `javascript:submitAction_win0(...)` postback). **City of Winnipeg confirmed working end-to-end (29/29, promoted to production.** The other 7 tenants don't auto-populate results on page load the way Winnipeg does — they need an explicit search submission first, and use a different row-element ID pattern (`HRS_VIEW_DETAILS$N`, no "PB" suffix, vs Winnipeg's `HRS_VIEW_DETAILSPB$N`). Added a search-button click + broadened selector for this, but it doesn't reliably trigger a results refresh — confirmed failing even when targeting the exact correct anchor element, on both a heavily-retested tenant (TMU) and a completely fresh one (Western), ruling out request-volume/rate-limiting as the cause. Root mechanism not yet understood — tabled for a future session.
