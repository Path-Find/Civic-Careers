...
- **Two reviewed job bodies no longer contain broken or repeated compensation text** — source-specific pension, transit-pass, leave, and employer-match details remain readable without duplicating sidebar benefits.
...
- **Qualifications no longer repeat Certifications** — 20 active postings were cleaned, and future parses keep mixed qualification bullets while removing certification-only restatements.
- **Qualifications no longer repeat Skills** — 36 active postings were cleaned, while mixed skill-and-context bullets remain visible.
- **Qualifications no longer repeat Languages** — 41 active postings were cleaned, while degree subjects, optional language preferences, and extra location/proficiency detail remain intact.
...
- **Expected start dates are now a structured field** (`start_date`): pulled from labels like Expected/Anticipated/Start date (calendar dates, Immediate, Fall 2026, etc.), shown on the job detail sidebar, filled for 886 existing postings, and extracted on new parses.
- **City tourism / employer pitch still opening Overview** (e.g. Cornwall’s “community of 47,000 on the St. Lawrence…”): Overview cleanup now strips fused and multi-paragraph municipal marketing even when the job title doesn’t appear in the body, plus a Cornwall source rule; 191 stored descriptions cleaned.
- **Driver’s / professional licences restated under Qualifications when already structured**: extraction now catches quoted class letters (`Class "G"`), conditions-of-employment walls, and “holder of …” wording; 659 rows refilled/stripped so licence bullets leave Qualifications once they sit in the licence field (and driver licences set `vehicle_required`).
- **Government of Canada inventory/process boilerplate still filling descriptions** (Work environment employer pitch, Intent of the process, Important messages, “not applying for a specific job”): stripped for GC sources and backfilled (~170 rows).
- **GC talent-pool postings (e.g. poster 2421444) stayed marked regular** because inventory detection required the exact phrase “inventory for future vacancies”; it now catches “not applying for a specific job but to an inventory” and related wording, and mis-marked pools were corrected.
- **CFIA/GC “pool of qualified candidates” postings stayed regular** (e.g. Veterinarian – Animal Health rid 557): inventory detection now covers that wording; body rebuilt with real duties/requirements and structured education, licence, language, and security fields.
- **High-school education and month-based experience were often left empty** (e.g. Brampton Parking & Sign Enforcement Officer): a list-splitter was breaking “Grade 12)” mid-phrase, and experience extraction only recognized years. Fixed, backfilled ~66 rows, and filled that posting’s hours, education, experience, COB permit, and open-till-filled listing type.
- **Education field carried federal junk prefixes** (`Education:`, `*Education:**`, `ED1:`, `AS-01 only:`, stream labels, screening glue): normalized off the structured field (109 rows) so Education shows just the credential.
