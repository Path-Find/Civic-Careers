...
### Fixed
- Fixed Vercel build failure — relative API imports (`_db`) need `.js` extension for `moduleResolution: nodenext`; package imports (`@libsql/client/http`) do not.

## [1.5.3] - 2026-07-03

### Fixed
- Fixed Vercel build failure — API imports missing `.js` extension required by `moduleResolution: nodenext` (`_db`, `@libsql/client/http`).

## [1.5.2] - 2026-07-03

### Fixed
- Fixed frontend build failure — `renderMarkdown`, `daysUntilClose`, and `fixCasing` in `utils.ts` updated to accept `string | null`; `Job` interface fields `job_title`, `department`, `location`, `description`, `closing_date` typed as nullable to match `job_details` LEFT JOIN.
