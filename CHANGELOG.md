...
### Fixed
- **Language requirements were stored in ~50 near-duplicate shapes** (`Bilingual` + `Bilingual (English/French)`, mixed sort order, `CBC level` vs `CBC/CBC`, multi-level "imperative" phrases as one blob). Normalization now collapses those into a small canonical set (including single-paren leveled bilingual: `Bilingual (English/French, CBC/CBC)` instead of nested parens), and existing rows were rewritten to match. New parses go through the same normalizer.
